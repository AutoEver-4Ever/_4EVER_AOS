...
@Composable
fun SupplierOrderScreen(
    navController: NavController,
    viewModel: SupplierOrderViewModel = hiltViewModel(),
) {
    ...
    Column(
        ...
        SearchBar(
            ...
            onQueryChange = { viewModel.updateSearchQuery(it, PurchaseOrderSearchTypeEnum.PurchaseOrderNumber) },
            ...
            is com.autoever.everp.utils.state.UiResult.Success -> {
                LazyColumn(
                    ...
                    items(orderList) { order ->
                        ListCard(
                            id = order.number,
                            title = "${order.supplierName} - ${order.itemsSummary}",
                            statusBadge = {
                                StatusBadge(
                            ...
                            details = {
                                Column(
                                    ...
                                    Text(
                                        text = "발주금액: ${formatCurrency(order.totalAmount)}원",
                                        style = androidx.compose.material3.MaterialTheme.typography.bodyMedium,
