...
import com.autoever.everp.data.datasource.remote.mapper.ProfileMapper
...
class ProfileRepositoryImpl @Inject constructor(
    ...
    override suspend fun refreshProfile(): Result<Unit> = withContext(Dispatchers.Default) {
        getProfile().map { profile ->
            profileLocalDataSource.setProfile(profile)
        }
    }

    override suspend fun getProfile(): Result<Profile> = withContext(Dispatchers.Default) {
